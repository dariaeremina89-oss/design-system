...
Search: `
## Назначение
Search вводит запрос и запускает поиск по кнопке «Найти» или Enter. Подходит для документов, организаций, сотрудников и поиска внутри Menu. Для выбора найденного значения используется Select; для обычного ввода — Input.
...
## Анатомия и размеры
Фон background-base-default, радиус radius-middle, тень shadow-m. Отступы сверху и снизу — 8 px. Минимальная ширина 112 px, исходное ограничение высоты — 304 px; maxHeight можно изменить. Список прокручивается, Search и область кнопок остаются на месте. Разделители добавляются только там, где они нужны по смыслу.
...
Menu находится в портале поверх содержимого и не меняет поток страницы. Расстояние до кнопки — 4 px. Клик снаружи, уход фокуса или Escape закрывают меню. После Escape и выбора действия фокус возвращается на вызывающий элемент; Tab продолжает обычную навигацию.

## Проверка
- [x] Клик и Enter/Space на кнопке раскрывают меню.
...
| searchable | Поиск прямо в поле Select |
| creatable | Сохранение собственного значения по Enter |
...
Значения options должны быть уникальными и непустыми. В поле показывается только label; описание опции остается в меню. Поиск не меняет выбранное значение до подтверждения. Созданное значение не добавляется в options. Escape отменяет поисковый ввод и сохраняет прежний выбор.

## Состояния и раскрытие
Default, Hover, Focused, Disabled и модификатор Error используют правила Input. При фокусе меняется обводка, геометрия остается прежней. Получение фокуса само по себе не открывает список. Клик, Enter, Space в обычном режиме и ArrowDown раскрывают Menu. Searchable открывается при вводе. Menu совпадает с шириной поля, располагается без зазора, перекрывает Caption и при нехватке места раскрывается вверх.
...
- [x] В поиске курсор остается в поле; aria-activedescendant указывает активный вариант.
