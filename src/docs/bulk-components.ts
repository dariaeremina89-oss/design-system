...
Длинные слова переносятся через overflow-wrap:anywhere; обычные слова остаются целыми. Высота строки определяется содержимым.
...
- Row перестраивается на мобильном, выбранные значения сохраняются.
- Control без подписи имеет aria-label; ошибку можно найти через aria-describedby.

### Уточнения по актуальному макету

Подпись опции — Subtitle 16/24 без верхнего отступа; иконка Checkbox — 20 × 20. Эти параметры заменяют Body 14, отступ 2 и иконку 24 из текстовой спецификации.
`;
...
Horizontal заполняет ширину, Vertical — высоту flex-контейнера. Inset задает внутренние отступы с обоих концов. Геометрия следует родительскому контейнеру. Макет содержит два размера Inset — 16 и 24; текстовое описание объединяет их в один вариант.

### Чеклист

- Линия толщиной 1 px, цвет токена border-base-tertiary.
- Full занимает доступное пространство; Insets сохраняются при изменении размера.
...
Gap: 0 или space-24. Group Divider показывается между соседними Accordion при нулевом gap независимо от раскрытия; при gap=24 разделители отсутствуют.
