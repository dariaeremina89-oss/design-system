...
export function SingleFileInput({
  ...
  const change = (e: ChangeEvent<HTMLInputElement>) => onFileChange?.(e.target.files?.[0] ?? null);

  return (
    <div {...props} className={`fdoc-single-file-input ${error ? 'fdoc-single-file-input--error' : ''} ${disabled ? 'fdoc-single-file-input--disabled' : ''} ${className}`} data-testid="single-file-input">
      <span className="fdoc-single-file-input__icon">
        <Icon name={error && !disabled ? 'filled/exclamation_circle_filled' : 'doc-paper'} size={24} />
      </span>
      <span className="fdoc-single-file-input__content">
        <span>{disabled ? 'Загрузка файлов недоступна' : placeholder}</span>
