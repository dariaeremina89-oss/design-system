...
export function SelectionControl({kind,label,description,error=false,errorText,state='default',indeterminate=false,disabled=false,id:providedId,className='',ref,onKeyDown,onChange,'aria-describedby':describedBy,...props}:Props) {
 ...
 const text=label!=null&&<span className="fdoc-control__text"><span className="fdoc-control__label" id={`${id}-label`}>{state==='skeleton'?<Skeleton width={120} height={12}/>:label}</span>{hint&&<span id={`${id}-hint`} className="fdoc-control__description" data-error={!!errorText}>{state==='skeleton'?<Skeleton width={160} height={8}/>:hint}</span>}</span>;
 if(state==='skeleton')return <span className={`fdoc-control-row ${className}`} data-kind={kind} aria-hidden="true">{visual}{text}</span>;
 return <label className={`fdoc-control-row ${className}`} data-kind={kind} data-disabled={inactive} data-state={state} htmlFor={id}>{visual}{text}</label>;
}
