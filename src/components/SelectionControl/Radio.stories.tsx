...
export const States:Story={render:()=> <div style={{overflowX:'auto',padding:8}}><table style={{borderSpacing:24}}><thead><tr><th>Вариант</th>{(['default','hover','focused','pressed','disabled','skeleton'] as const).map(s=><th key={s}>{s}</th>)}</tr></thead><tbody>{[false,true].flatMap(error=>[false,true].map(checked=><tr key={`${error}-${checked}`}><th>{checked?'Selected':'Unselected'}{error?' / Error':''}</th>{(['default','hover','focused','pressed','disabled','skeleton'] as const).map(state=><td key={state}><Radio aria-label={`${checked} ${error} ${state}`} checked={checked} onChange={()=>{}} error={error} state={state} /></td>)}</tr>))}</tbody></table></div>};
