...
export function Pagination({totalElements,size,page,onChangePage,direction='horizontal',color='base',showCounter=true,isLoading=false,className=''}:PaginationProps) {
 const total=Number.isFinite(totalElements)?Math.max(0,Math.floor(totalElements)):0;const perPage=Number.isFinite(size)&&size>0?Math.max(1,Math.floor(size)):1;const pages=Math.ceil(total/perPage);const current=Math.max(1,Math.min(pages||1,Math.floor(page)||1));const root=useRef<HTMLElement>(null);const probe=useRef<HTMLSpanElement>(null);const [width,setWidth]=useState(Infinity);const [buttonWidth,setButtonWidth]=useState(32);
 ...
 {showCounter&&<div className="fdoc-pagination__counter">{isLoading?<Skeleton width={120} shape="text" textSize="body"/>:`${(current-1)*perPage+1}—${Math.min(current*perPage,total)} из ${total}`}</div>}
 {pages>1&&<div className="fdoc-pagination__pages">{pages>=5&&(isLoading?<Skeleton width={32} height={32}/>:<ButtonPagination iconLeft="arrow-chevron-left" color={color} disabled={current===1} aria-label="Предыдущая страница" onClick={()=>onChangePage?.(current-1)}/>)}{visible.map((n,i)=>n==='ellipsis'?<span key={`ellipsis-${i}`} className="fdoc-pagination__ellipsis" aria-hidden="true">{isLoading?<Skeleton width={16} shape="text" textSize="caption"/>: '…'}</span>:isLoading?<Skeleton key={n} width={buttonWidth} height={32}/>:<ButtonPagination key={n} color={n===current?'primary':color} aria-label={`Страница ${n}`} aria-current={n===current?'page':undefined} onClick={()=>{if(n!==current)onChangePage?.(n);}}>{n}</ButtonPagination>)}{pages>=5&&(isLoading?<Skeleton width={32} height={32}/>:<ButtonPagination iconLeft="arrow-chevron-right" color={color} disabled={current===pages} aria-label="Следующая страница" onClick={()=>onChangePage?.(current+1)}/>)}</div>}
 </nav>;
}
