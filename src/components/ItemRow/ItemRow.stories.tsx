...
export const States:Story={render:args=><div style={{display:'grid',gap:12}}>{(['default','hover','pressed','focused','disabled','skeleton'] as const).map(state=><ItemRow {...args} key={state} state={state} title={state} description="Описание" leadingIcon="copy" helper="PDF"/>)}</div>};
export const Variants:Story={render:args=><><ItemRow {...args} variant="header" title="Заголовок группы"/><ItemRow {...args} description="Дополнительное описание"/><ItemRow variant="link" title="Подробнее" href="#details"/><ItemRow variant="search" searchProps={{'aria-label':'Поиск по списку'}}/></>};
