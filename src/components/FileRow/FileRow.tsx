...
export function FileRow({
  ...
  return (
    <div {...props} className={`fdoc-file-row fdoc-file-row--${type} ${error ? 'fdoc-file-row--error' : ''} ${className}`} data-testid="file-row">
      {draggable && <span className="fdoc-file-row__drag"><Icon name="drag-dot" size={24} /></span>}
      {previewed ? (
        <span className="fdoc-file-row__preview">{preview ?? <span className="fdoc-file-row__preview-placeholder" />}</span>
      ) : (
        <span className="fdoc-file-row__leading">
          {leadingView ?? (loading ? <ProgressIndicator type="circular" mode="indeterminate" size={20} variant="secondary" /> : <Icon name={icon} size={24} />)}
        </span>
      )}
      <div className="fdoc-file-row__content">
        <div className="fdoc-file-row__line">
          <span className="fdoc-file-row__name">{fileName}</span>
          <span className="fdoc-file-row__meta">
            ...
            {trailingView ?? (deletable && <ButtonIcon aria-label="Удалить файл" icon="filled/cross_circle_filled" size="xsmall" iconSize={16} color="neutral" state={disabled ? 'disabled' : 'default'} onClick={onDelete} />)}
          </span>
        </div>
